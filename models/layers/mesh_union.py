...
class MeshUnion:
    def __init__(self, n, device=torch.device('cpu')):
        ...
        idxs = torch.arange(n)
        self.sparse_groups = torch.sparse.FloatTensor(torch.stack([idxs, idxs]), torch.ones(n)).to(device)
    ...
    def union(self, source, target):
        ...
        assert type(source) in [np.int64, int] and type(target) in [np.int64, int]

        # All of this mess is just adding two rows
        # equivalent to self.groups[target, :] += self.groups[source, :]
        idxs = self.sparse_groups.indices()
        source_mask = idxs[0,:] == source
        source_columns = idxs[1, source_mask]
        target_rows = torch.ones(source_mask.sum(), dtype=torch.int64).to(self.sparse_groups.device)*target
        source_target_idxs = torch.stack([target_rows, source_columns])

        values = self.sparse_groups.values()[source_mask]

        all_values = torch.cat([self.sparse_groups.values(), values], dim=0)
        all_idxs = torch.cat([self.sparse_groups.indices(), source_target_idxs], dim=1)
    ...
    def get_groups(self, tensor_mask):
        ...
        value_mask = isin(self.sparse_groups.indices()[0, :], where_mask)

        new_cols = self.sparse_groups.indices()[1,value_mask]
        # Rescale row_idx to slice
        temp = self.sparse_groups.indices()[0,value_mask]
        ...
        return torch.sparse.FloatTensor(idxs, self.sparse_groups.values()[value_mask], (tensor_mask.sum(), self.sparse_groups.shape[1])).to_dense()
    ...
    def rebuild_features_average(self, features, mask, target_edges):
        ...
        if not self.sparse_groups.is_coalesced():
            self.sparse_groups = self.sparse_groups.coalesce()
        idxs, values = transpose(self.sparse_groups.indices(), self.sparse_groups.values(), self.sparse_groups.shape[0], self.sparse_groups.shape[1])
    ...
    def prepare_groups(self, features, mask):
        ...
        values = self.sparse_groups.values()[value_mask]
        indicies = self.sparse_groups.indices()[:,value_mask]
